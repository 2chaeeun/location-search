...
* 확장성을 위해서 List를 입력 값으로 받는다. placesList에 병합할 리스트를 담는다.
* 리스트에 먼저 넣을수록 우선순위가 높다.
* 만약에 카카오, 네이버, 구글 순으로 넣는다면 많이 나온 장소가 상위에 오고, 나온 횟수가 같다면 카카오, 네이버, 구글 순으로 정렬된다.
  ```
  ...
  ```
* 결과를 담을 리스트(results)를 만든다. 첫번째 리스트는 results에 다 넣는다. 다음 리스트의 원소를 results의 원소와 매칭하여 동일한 원소일 경우에 results의 원소 카운트를 증가시키고,
  같은 카운트의 원소가 나올 때까지 앞으로 이동시킨다.
  ```
...
* 두 장소 간 동일 업체 판단 기준:
  ...
  ```
    ...
    * 업체명:
        * 업체명에 있는 모든 숫자가 같은지 확인합니다. 하나라도 다르거나 없으면 다른 장소라고 판단합니다.
        * 라벤슈타인 편집 거리가 4초과인 경우는 다른 장소라고 판단합니다.
            * ex) 남산돈가스, 남산돈까스전문점 같은 장소인데 업체명이 다릅니다. 4글자가 달라도 같은 장소라고 볼 수 있습니다.
    * 카테고리:
